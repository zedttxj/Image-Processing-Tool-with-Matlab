...
We can use tree-based modeling to generate the **full** decomposition of the input partition. In the below demonstration, the "Left" groups are guaranteed constant partitions and only have 1 partitions. The "Right" groups may contain multiple partitions. Let's consider a simple case first:
```mermaid
graph TD;
    C[3 3 2 1] --> A[1 1];
    C[3 3 2 1] --> B[2 2 1];
