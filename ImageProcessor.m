classdef ImageProcessor
    methods(Static)
        ...
        function bayerImage = test(rgbImage, filter, show, rgb, filtersize, ord)
                ...
                if rgb(k)
                    ...
                    for i = 1:rf:ri
                        for j = 1:cf:ci
                            i_end = min(i + rf - 1, ri);
                            j_end = min(j + cf - 1, ci);
                            bayerImage(i:1:i_end, j:1:j_end) = bayerImage(i:i_end, j:j_end) + rgbImage(i:i_end, j:j_end, k) .* mask(1:(i_end-i+1), 1:(j_end-j+1));
                        end
                    end
                end
            end
